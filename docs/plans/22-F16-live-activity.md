...
## 후속 · 워치에서 중단했는데 카드가 남는다 (fix/live-activity-leak)

### 무엇이 일어나는가

워치에서 세션을 중단하면 최종 스냅샷이 폰으로 가고(`transferUserInfo`), 수신 경로가
`refresh(for:)` 를 불러 카드를 끝낸다. **폰이 그때 깨어난다면** 그렇다.

폰 앱이 강제 종료돼 있으면 iOS 는 백그라운드 전달로 앱을 깨우지 않는다. 그러면 종료
신호가 도착하지 않고, 시스템이 소유한 카드는 그대로 남는다. 운동이 끝났는데 잠금화면은
진행 중으로 보인다 — 이 앱이 그동안 겪은 문제의 모양 그대로다(D14).

### 고친 방법

**앱이 앞으로 나올 때 저장소와 맞춘다.** 살아 있는 세션이 없으면 카드도 끝낸다.
계획 21 이 유령 세션을 앱 열 때 정리하는 것과 같은 장치이고, 같은 이유로 필요하다 —
전달에 의존하는 경로는 언젠가 새고, 그때 받아줄 그물이 있어야 한다.

`staleDate` 도 함께 넣었다. 그물에 걸리기 전까지(폰을 열기 전까지) 카드가 **최신인
척하지 않게** 한다. 세션 하나가 평균 50분이라 2시간으로 잡았다.

### 남겨두는 것

**워치용 위젯은 만들지 못한다.** 위젯 확장은 별도 프로세스라 워치 앱의 SwiftData
저장소를 보려면 App Group 이 필요한데, **무료 계정에서는 쓸 수 없는 기능이다**(D5).
유료 계정으로 전환하면 그때 만든다.

대신 `HKWorkoutSession`(계획 21)과 워치 앱 자동 실행(F-17)이 같은 필요를 채운다 —
손목을 들면 이미 세트 화면이라 스마트 스택 카드를 누를 일이 없다.
