...
잡혀 있는데, 작업 단위로 옮길 때 "복사 버튼"만 남고 상세 화면이 빠졌다. 목록 행에 복사
버튼을 붙이는 것으로 "복사는 된다"가 충족돼 보여 검증에서도 걸리지 않았다.

실사용에서 기록을 눌러도 아무 반응이 없어 드러났다. 작업 단위가 요구사항 본문을 온전히
옮겼는지 확인하지 않으면 이런 누락은 체크박스가 전부 채워진 상태로 지나간다.

### 8. `SessionDetailView` 에 담을 것

- 종목별 세트 목록 — 목표 `무게 × 횟수`, 결과(✅ / ❌ + 실제 횟수 / ⏭ / 미수행은 빈 칸), 휴식 시간
- 요약 — 소요 시간, 완료 비율, 평균 휴식. `SessionCompleteView` 와 같은 지표를 쓴다
- 마크다운 복사 — 목록 행의 버튼을 여기로 옮긴다. 목록은 훑어보는 곳이고 복사는 상세에서 한다
- 중단된 세션도 열람 가능해야 한다(PRD F-3)

표시에 필요한 값은 `WorkoutSession` 에 이미 다 있다. 새로 만들 도메인 로직은 없다.
