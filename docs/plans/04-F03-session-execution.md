...
이 화면들은 F-08(폰↔워치 동기화)이 없는 상태에서 만들어졌다. 당시 워치는 자기 로컬
저장소만 읽고 쓸 뿐, 폰의 루틴을 받아올 방법이 없었다 — 그래서 실기기 검증 대신, 워치
로컬 저장소에 직접 씨드 데이터를 넣어 "시작 → 성공 → 실패(Digital Crown 기본값 + 무게
스테퍼) → 되돌리기 → 성공 → 완료" 전 과정을 워치 시뮬레이터에서 확인했다.

**갱신(F-08 이후).** `WCSession` 배선은 끝났다 — `WatchRootView` 는 이제 폰에서 내려온
루틴을 받아 보여주고, `WatchSetView` 는 세트를 기록할 때마다 결과를 폰으로 올려보낸다
([08-F08](08-F08-watch-sync.md) "화면·앱 통합" 참고). 다만 이 문서의 완료 기준 2번
(실제 폰 ↔ 워치 왕복)은 여전히 실기기 확인이 남아 있고, F-08 문서에 적어둔 대로
"세션을 서로 이어받기"는 아직 없다.
