...
- ✅ `LastRecordLookup` — 종목별 직전 기록 조회. **여기서 직접 호출한다**

**07-F09 를 기다리지 않는다.** 조회 로직은 완료돼 있다.
07-F09 는 통합 결과를 점검하는 문서이지 선행 작업이 아니다.

## 작업 단위

- [x] 1. `RoutineScheduler` — 요일 배타 배정 로직 (WoofitCore)
- [x] 2. `ExerciseNameSuggester` — 과거 종목명 수집·정렬 (WoofitCore)
- [x] 3. `Routine.duplicate()` — 루틴 복제 (WoofitCore)
- [x] 4. 테스트 (1~3)
- [x] 5. `RoutineEditorView` — 이름·카테고리·요일
- [x] 6. 종목 목록 — 추가·순서 변경·삭제
- [x] 7. 세트 편집 — 무게/횟수 입력, 세트 복제
- [x] 8. 자동완성 연결
- [x] 9. 루틴 목록에서 편집·복제·삭제 진입
...
**2·3은 아직 사람이 시뮬레이터에서 직접 확인하지 못했다.** 이번 세션에서는 접근성
자동화 권한이 없어 탭을 스크립트로 보낼 수 없었다 — 빌드·앱 실행·초기 화면 렌더링까지만
확인했다. 배타 배정·빈 종목 경고·2분 작성 체감은 사람이 시뮬레이터를 직접 조작해
확인해야 한다.

## 빈 종목 처리 — 착수 전 확정

**세트가 0개인 종목은 저장 시점에 막는다.** 편집기에서 종목만 추가하고 세트를 넣지 않은
채 저장하려 하면 경고하고, 저장하지 않거나 그 종목을 빼고 저장한다.

허용하면 마크다운 왕복이 깨진다. 내보내기는 `| 벤치프레스 | | 0 |` 같은 빈 행을 쓰는데,
가져오기는 `setCount > 0` 이 아니면 그 행을 건너뛴다(§6.5). 내보냈다 다시 가져오면
종목이 조용히 사라진다 — 오류로 드러나지 않아 눈치채기 어렵다.

`SessionExercise.isComplete` 는 빈 종목을 완료로 보므로 세션이 막히지는 않는다.
그 수정은 세션이 빠져나오지 못하는 버그를 막으려는 것이었고, 애초에 빈 종목이
루틴에 남지 않게 하는 것은 이 계획의 몫이다.
