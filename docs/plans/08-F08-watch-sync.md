...
## 후속 · 워치에 루틴이 없다 (fix/watch-routines-missing)

### 무엇이 일어나는가

워치 앱을 열면 "루틴 없음"이 뜨는 일이 잦았다. 원인이 둘 맞물린다.

**① 같은 내용의 컨텍스트는 전달되지 않는다.** `updateApplicationContext` 는 직전과 동일한
딕셔너리를 보내면 상대의 `didReceiveApplicationContext` 가 아예 불리지 않는다.

**② 앱을 재설치하면 워치 저장소가 빈다.** 루틴도, 받아둔 컨텍스트도 사라진다.

그래서 재설치 → 워치 루틴 0개 → 폰이 **똑같은 payload** 를 보냄 → 전달 안 됨 →
루틴을 하나 고치기 전까지 복구되지 않는다.

### 고친 방법

**보내는 쪽** — 컨텍스트를 고치는 통로를 하나(`updateContext`)로 모으고 매번 달라지는
`sentAt` 을 심는다. 별도 키라 루틴·세션 payload 자체는 그대로고, 받는 쪽은 payload 가
같으면 병합을 건너뛰므로 헛일도 하지 않는다.

**받는 쪽** — 워치가 루틴이 하나도 없으면 폰에 다시 보내달라고 요청한다.
`transferUserInfo` 를 쓰므로 **폰 앱이 꺼져 있어도 깨워서** 도착한다. 받기만 기다리면
폰을 열 때까지 헬스장에서 빈 화면을 보고 있어야 한다.

둘 다 필요하다. ①만 고치면 폰 앱을 열어야 복구되고, ②만 고치면 요청에 대한 응답이
직전과 같은 내용이라 또 전달되지 않는다.
