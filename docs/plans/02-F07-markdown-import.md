...
- [x] 1. `ParsedRoutine` · `ParsedExercise` — SwiftData 가 아닌 값 타입. 미리보기용
- [x] 2. `ParseIssue` — 건너뛴 줄과 이유
- [x] 3. `MarkdownTableParser` — 파이프 표를 행·셀로 분해
- [x] 4. 형식 판별 — 헤더 행을 보고 A / B / 루틴 구분
- [x] 5. 메타데이터 파싱 — 제목, `- 부위:`, `- 반복:`, 제목에서 요일 추론
- [x] 6. `목표` 열 파싱 — `80kg × 5`, `맨몸 × 15`, `22.5kg × 5`, `70~80kg × 5`(범위, 세트 수만큼 균등 보간)
- [x] 7. 세트 수 판정 — 형식별 규칙
- [x] 8. 테스트 (왕복 테스트 포함)
- [x] 9. 폰 UI — 붙여넣기 → 미리보기 → 적용
...
| `70~80kg × 5` 범위를 세트 수만큼 균등 보간한다 | 피라미드 세트 왕복(PRD §6.5) |
| 범위 표기이고 세트가 1개면 낮은 쪽 값을 쓴다 | 보간 경계 조건 |
...
- **피라미드 세트 왕복은 근사치다.** `RoutineMarkdownExporter.targetLabel` 이 세트별 무게가
  다르면 `WeightFormatter.targetRange` 로 범위 문자열(`70~80kg × 5`)로 뭉갠다. 원본 세트별
  무게는 문자열만으로 복원할 수 없으므로, 가져올 때는 세트 수만큼 낮은 값에서 높은 값까지
  균등 보간한 근사치를 만든다(PRD §6.5). 완벽한 복원이 아니라 재편집 없이 쓸 수 있는
  근사치가 목표다.
