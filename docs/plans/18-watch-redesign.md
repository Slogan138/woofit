...
- [x] 11. `WatchFailureInputView` 의 큰 숫자도 같은 토큰 경유로 바꾼다 — 지금은
      화면이 직접 폰트를 지정한다
- [x] 12. 실기기에서 확인 — **Apple Watch Ultra 3(49mm)**, 손목 각도, Always-On 상태

> **이 문서의 41mm 기준은 실제 기기와 다르다.** 사용자의 기기는 Ultra 3 (49mm) 다(D15).
> 좁은 화면을 전제로 내린 판단(고정 액션 바 대신 순서 재배치, 36pt 판독값)은 넓은
> 화면에서 문제가 되지 않으므로 되돌리지 않는다 — 이미 손에 익은 위치를 흔들 이유가 없다.
> 앞으로 워치 화면을 손볼 때는 **49mm 를 기준으로 판단한다.**
