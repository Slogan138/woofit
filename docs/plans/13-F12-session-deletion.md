...
### 실제 원인은 `role: .destructive` 였다

위 네 가지를 고친 뒤에도 증상이 남았다. **정지 스크린샷으로는 잡히지 않는다** —
팝업이 다 뜬 뒤엔 행이 이미 제자리로 돌아와 있다. 시뮬레이터 화면을 녹화해 0.1초
간격 프레임으로 쪼개고, 각 행의 진행 막대 y 좌표를 추적해서야 드러났다.

```
수정 전                                  수정 후
0.0~4.3s  y=[834,1109,1384,1659]        0.0~4.7s  y=[834,1109,1384,1659]
4.4s      y=[834,1109,1333,1608]        4.8s      팝업이 1행을 가린 것뿐
4.5s      y=[834,1109,1176,1452]  ←208px 이동
4.6s      y=[1109,1395]           ←2행이 접힘
4.8s      원위치 복귀
```

스와이프 액션에서 `role: .destructive` 는 색 지정이 아니라 **"이 버튼을 누르면 이 행이
없어진다"는 신호**다. SwiftUI 가 데이터를 확인하지 않고 행을 접는다. 이 화면은 확인을
한 번 더 받으므로 그 시점엔 지워진 것이 없고, 그래서 접혔다가 되돌아온다.

`.tint(.red)` 로 바꾸면 같은 빨간 버튼이면서 행을 접지 않는다.

**교훈** — 애니메이션 결함은 코드를 읽어서도, 정지 화면을 봐서도 판정할 수 없다.
이 건은 원인을 세 번 짚어 두 번 빗나갔고, 프레임을 실측하고 나서야 확정됐다.
